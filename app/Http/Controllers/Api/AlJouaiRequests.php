...
class AlJouaiRequests
{
    ...
    public static function SQL_get_missing_data($phone, $invoicesTuple)
    {
        $missingQuery  =  "
        SELECT
...
WHERE M0.Phone = '" . $phone . "' AND T0.CANCELED ='N' AND T0.DocEntry NOT IN (".$invoicesTuple.")
";

        $data  = [];
        $stmt = self::establishConnectionDB($missingQuery);
    ...
    // 
    public static function SQL_get_data_range($phone, $startDate, $endDate)
    {
        ...
        WHERE M0.Phone = '" . $phone . "' AND T0.CANCELED ='N' AND T0.DocDate >= '" . $startDate . "' AND T0.DocDate <= '" . $endDate . "')
        SELECT
        T0.DocEntry, CASE WHEN ISNULL(T0.LicTradNum, '') = '' THEN N'فاتورة ضريبية مبسطة'
        ELSE N'فاتورة ضريبية' END AS 'InvoiceTitle',
        T0.CardName, T0.CardCode , M0.Phone ,T0.LicTradNum , T0.DocDate , T0.DocDueDate ,
...
";
    ...
    public static function SQL_get_data_specific_date($phone, $specificDate)
    {
        $qer = "
        WITH SpecificDate AS (SELECT T0.DocEntry 
FROM AljouaiT.DBO.OINV T0 LEFT JOIN AljouaiT.DBO.[@MobileNumber] M0 ON T0.DocEntry = M0.DocEntry  
WHERE M0.Phone = '" . $phone . "' AND T0.CANCELED ='N' AND T0.DocDate = '" . $specificDate . "' )
SELECT
T0.DocEntry, CASE WHEN ISNULL(T0.LicTradNum, '') = '' THEN N'فاتورة ضريبية مبسطة'
ELSE N'فاتورة ضريبية' END AS 'InvoiceTitle',
T0.CardName, T0.CardCode , M0.Phone ,T0.LicTradNum , T0.DocDate , T0.DocDueDate ,
...
";
    ...
    public static function getLastFiveInvoices($phone)
    {
        $qer = "
